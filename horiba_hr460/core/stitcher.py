...
class SpectrumStitcher:
    ...
    (replicates and modernizes legacy frmGlue functionality).
    """
    ...
    def plan_intervals(
        ...
        # Handle grating dispersion direction (increasing or decreasing wavelength across array)
        ...
        while (curr_right < to_nm if increasing else curr_right > to_nm) and idx < 100:
            idx += 1
            # Next center placing the previous edge at the overlap pixel
    ...
    def stitch_spectra(
        ...
        if step_nm is None:
            # Estimate native step size from first window
