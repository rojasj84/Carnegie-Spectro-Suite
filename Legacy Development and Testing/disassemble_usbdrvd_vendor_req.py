...
dll_path = r"C:\Users\LabUserR129\Desktop\WinSpec32_Full\USBDRVD.DLL"
with open(dll_path, "rb") as f:
    data = f.read()

# Find export USBDRVD_VendorOrClassRequestOut
...
for i in range(num_names):
    ...
    if "VendorOrClass" in name or "Bulk" in name:
        print(f"Export '{name}' -> RVA 0x{frva:08X} (Offset 0x{foff:04X})")
        # Print 50 bytes of code
        chunk = data[foff:foff+50]
        print(f"  Code hex: {chunk.hex()}")
